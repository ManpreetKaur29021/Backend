...
{
    if(!empty($_POST['title']) && !empty($_POST['status']) && !empty($_POST['count']))
    {
        $title = $_POST['title'];
        $status = $_POST['status'];
        $count = $_POST['count'];
        $q = "select * from button where title='$title' and status='$status'";
    ...
    else if(!empty($_POST['title']) && !empty($_POST['status']))
    {
        $title = $_POST['title'];
        $status = $_POST['status'];
        $q = "select * from button where title='$title' and status='$status'";
    ...
    else if(!empty($_POST['title']) && !empty($_POST['count']))
    {
        $title = $_POST['title'];
        $count = $_POST['count'];
        $q = "select * from button where title='$title'";
        $res = mysqli_query($con,$q);
        if(mysqli_num_rows($res)>0)
        {
            $a=1;
    ...
    else if(!empty($_POST['status']) && !empty($_POST['count']))
    {
        $status = $_POST['status'];
        $count = $_POST['count'];
        $q = "select * from button where status='$status'";
        $res = mysqli_query($con,$q);
        if(mysqli_num_rows($res)>0)
        {
            $a=1;
...
?>
        ...
        <h5 class="empty-table-message" style="text-align: center;background-color:#d3dbd5;padding: 50px;">No Payments Found!</h5>
       </div>
        <?php } ?>
        </div>
    </div>
